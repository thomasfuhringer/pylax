...
def entryCustomer__verify(widget, data):
    rows = selector.dynaset.execute({"Name": data}, "SELECT LeID, Name FROM LE WHERE Name = :Name;")
    if rows == 1:
        ...
        if not selector.show():
            return False
    return {"Customer": selector.dynaset.get_data("LeID"), "CustomerName": selector.dynaset.get_data("Name")}
