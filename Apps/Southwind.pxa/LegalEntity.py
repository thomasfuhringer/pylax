...
def launch():
    ...
    ds = pylax.DataSet("LE", "SELECT le.LeID, le.Name, le.Description, le.Logo, le.Parent, p.Name AS ParentName FROM LE LEFT JOIN LE AS p ON le.Parent=p.LeID WHERE le.Name LIKE :Name OR le.LeID = :Name ORDER BY le.Name DESC LIMIT 100;")
    ...
    labelFormCaption = pylax.Label(form, 1, 1, 40, 20, dataSet=ds, column="Name", visible=False)
    ...
    imagePicture = pylax.ImageView(form, -140, 20, 90, 80, dataSet=ds, column="Logo", dataType=bytes)

    entryID = pylax.Entry(form, -160, 120, 60, 20, dataSet=ds, column="LeID", dataType=int, label = pylax.Label(form, -230, 120, 60, 20, "ID"))
    entryID.editFormat="{:,}"
    entryID.alignHoriz = pylax.Align.left

    entryName = pylax.Entry(form, -160, 150, 140, 20, dataSet=ds, column="Name", label = pylax.Label(form, -230, 150, 60, 20, "Name"))
    entryParent = pylax.Entry(form, -160, 180, 140, 20, dataSet=ds, column="ParentName", label = pylax.Label(form, -230, 180, 60, 20, "Parent"))
    #entryParent.alignHoriz = pylax.Align.left
    #entryParentName = pylax.Entry(form, -160, 210, 100, 20, dataSet=ds, column="ParentName")
...
def entryParent__verify(self):
    r=selector.dataSet.execute({"Name": self.inputString})
    if r == 1:
        print(selector.dataSet.get_row_data(0)["LeID"])
        selector.dataSet.row = 0
        self.input = selector.data["Name"]
        self.dataSet.set_data("Parent", selector.data["LeID"])
        return True
    if selector.run():
...
def create_selector_LegalEntity():
    ...
    ds = pylax.DataSet("LE", "SELECT LeID, Name, Description FROM LE WHERE Name LIKE :Name OR LeID = :Name ORDER BY Name DESC LIMIT 100;")
    ...
    table = pylax.Table(selector, 20, 48, -20, -60, dataSet=ds)
